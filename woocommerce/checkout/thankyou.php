...
global $woocommerce;
if ($order) {
$related=ThemexWoo::getRelatedPost($order->get_id(), 'tour');
if(!empty($related)) {
$GLOBALS['columns']=0;
$GLOBALS['width']='';
$GLOBALS['counter']=0;
	...
	<div class="tour-checkout fivecol column">
		<?php if ( in_array( $order->get_status(), array( 'failed' ) ) ) : ?>
			...
			<p>
				...
				<a href="<?php echo esc_url( get_permalink( wc_get_page_id( 'myaccount' ) ) ); ?>" class="button pay"><?php _e( 'My Account', 'midway' ); ?></a>
				<?php endif; ?>
			</p>
		<?php else : ?>
